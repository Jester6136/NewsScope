            ...
            if ALLOW_TRIGGER1:
                ...
                for argument_type in arguments:
                    ...
                    if item[answer_key] is not None:
                        ...
                        data_format["qas"].append(qas_format)
            if item["trigger_2"] and ALLOW_TRIGGER2:
                trigger_question2 = TRIGGER_GENERATE(trigger1=item["trigger_1"][1],subject1=item["subject_1"][1],object1=item["object_1"][1])
                data_format["qas"].append({"answers": [{"answer_start": item["trigger_2"][0], "text": item["trigger_2"][1]}], "question": trigger_question2})
                arguments = ["subject", "object", "time", "place"]
                
                for argument_type in arguments:
                    question_func = globals()[f"{argument_type.upper()}_GENERATE"]
                    question = question_func(item["trigger_2"][1])
                    answer_key = f"{argument_type}_2"
